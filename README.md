...
# Build from source
- Get DSM toolkit for your model according to Synology [Developer's guide](https://developer.synology.com/developer-guide/)
...
- Workaround DSM toolkit missing command `group`, which Privoxy build process needs
  ```sh
  cp /usr/bin/groups /toolkit/build_env/ds.evansport-6.1/usr/bin
  ```
- Create user and group privoxy into the chroot
  ```
  chroot /toolkit/build_env/<your target env>
  ```
  then edit `/etc/passwd` and `/etc/group`
